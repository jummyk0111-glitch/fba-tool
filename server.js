...
function renderResults(data) {
  ...
  data.products.forEach(p => {
    ...
    html += \`<div class="product-card">
      ...
      \${p.complementary_products?.length ? \`<div class="analysis-section" style="margin-top:16px"><div class="analysis-label" style="color:#10b981">5 Complementary Products</div><div class="comp-grid">\${p.complementary_products.map(c=>\`<div class="comp-card"><div class="comp-name">\${c.name}</div><div class="comp-reason">\${c.reason}</div><div class="comp-badges"><span class="comp-badge comp-badge-cost">Cost: \${c.estimated_cost}</span><span class="comp-badge comp-badge-impact">Impact: \${c.impact}</span></div></div>\`).join('')}</div></div>\` : '<div class="analysis-section"><div class="analysis-label" style="color:#10b981">Complementary Products</div><div class="analysis-text">No complementary products returned. Please run a new analysis.</div></div>'}
    </div>\`;
  });
